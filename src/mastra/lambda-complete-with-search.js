...
'your-neptune-cluster.cluster-xxxxx.us-east-2.neptune.amazonaws.com';
...
exports.handler = async (event) => {
    ...
    try {
        ...
        switch (operation) {
            ...
            case 'searchEntitiesByName':
                return await searchEntitiesByName(event, g);
...
async function getEntityRelationships(event, g) {
    ...
    try {
        ...
        const outgoing = await g.V().has('entityId', entityId)
            ...
            .by('label')  // Changed from T.label to 'label'
...
async function searchEntitiesByName(event, g) {
    ...
    try {
        ...
        // Use Gremlin's TextP for text searching
        const TextP = gremlin.process.TextP;
        const searchLower = searchTerm.toLowerCase();
        
        // First try to find exact and partial matches in name
        let entities = await g.V()
            .hasLabel('entity')
            .or(
                // Exact match (case-insensitive)
                __.has('name', searchTerm),
                // Contains match using TextP
                __.has('name', TextP.containing(searchTerm)),
                __.has('name', TextP.containing(searchLower)),
                // Search in entityId (e.g., entity_person_montag)
                __.has('entityId', TextP.containing(searchLower)),
                // Search in description
                __.has('description', TextP.containing(searchTerm)),
                __.has('description', TextP.containing(searchLower))
            )
            .limit(limit)
            .valueMap('entityId', 'name', 'entityType', 'description')
            .toList();
        
        // If no results, try word-by-word matching for multi-word searches
        if (entities.length === 0 && searchTerm.includes(' ')) {
            const words = searchTerm.split(/\s+/);
            for (const word of words) {
                if (word.length > 2) {  // Skip very short words
                    const wordMatches = await g.V()
                        .hasLabel('entity')
                        .or(
                            __.has('name', TextP.containing(word)),
                            __.has('name', TextP.containing(word.toLowerCase()))
                        )
                        .limit(limit)
                        .valueMap('entityId', 'name', 'entityType', 'description')
                        .toList();
        ...
        console.log(`Found ${entities.length} entities matching "${searchTerm}"`);
